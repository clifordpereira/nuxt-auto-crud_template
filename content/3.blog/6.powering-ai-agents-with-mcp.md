...
2.  **API Mapping**: Agents know the pluralized endpoints (`/api/_nac/users` vs `/api/_nac/people`) without trial and error.
...
It works by introspecting the new `/api/_nac/_meta` endpoint, which returns a comprehensive manifest of your entire database-driven API.
...
```json
{
  "mcpServers": {
    "nac-schema-inspector": {
      ...
      "env": {
        "ALLOWED_URLS": "[\"http://localhost:3000/api/_nac/_meta?format=md\"]"
      }
    }
  }
}
