...
In addition to standard actions, you can assign **Ownership Permissions** for granular control:

*   **`list`**: View a list of *active* records.
*   **`list_all`**: View *all* records, including inactive ones (e.g., status='draft').
*   **`list_own`**: View **only** records created by the user.
*   **`update_own`**: Update a record **only if** the user created it.
*   **`delete_own`**: Delete a record **only if** the user created it.

### Prerequisites
For ownership permissions to work, the table must track who created the record. Ensure your schema includes a `createdBy` field:
...
The system automatically checks if `createdBy` matches the logged-in user's ID.
