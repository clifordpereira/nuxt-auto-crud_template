...
To add a new table, simply create a new file. For example, `server/db/schema/posts.ts`:
...
You must export all your schema files in `server/db/schema/index.ts` so Drizzle can find them.
