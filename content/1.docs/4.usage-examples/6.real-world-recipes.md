...
- **POST /api/_nac/subscribers**: Public users can submit `{ "email": "..." }`.
- **GET /api/_nac/subscribers**: Public users will receive `403 Forbidden` (protecting your list).
...
Since the generic `/api/_nac/testimonials` endpoint might be restricted or return all items (including inactive ones if public has list access), we create a specific endpoint for the frontend display.

```typescript
// server/api/_nac/active-testimonials.get.ts
...
const { data: testimonials } = await useFetch('/api/_nac/active-testimonials')
</script>
```
...
The `CrudCreateRow` component automatically handles the POST request to `/api/_nac/testimonials`.
...
const { data: roles } = await useFetch<any[]>('/api/_nac/roles')
